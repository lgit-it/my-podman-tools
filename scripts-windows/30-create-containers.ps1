...
# Uso: .\scripts-windows\30-create-containers.ps1 [-Containers <container1,container2,...>] (esegui come Amministratore)

param(
    [string[]]$Containers = @("postgres", "odoo", "n8n", "nginx")
)
...
if ($BIND_LOCALHOST -eq "1") {
    ...
    $ODOO_DEBUG_BIND = "127.0.0.1:${ODOO_DEBUG_HOST_PORT}:5678"
    ...
    $ODOO_DEBUG_BIND = "${ODOO_DEBUG_HOST_PORT}:5678"
...
Write-Log "Container creati: $containerList" -Level "SUCCESS"
Write-Log "Avvio manuale: podman start $containerList"
